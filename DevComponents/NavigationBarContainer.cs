...
namespace DevComponents.DotNetBar
{
    ...
    public class NavigationBarContainer : ImageItem, IDesignTimeProvider
	{
		...
		public void ShowMoreButtons()
		{
            ...
            if (ctrl != null)
                ctrl.Height += GetChangeSizeHeight(false) + (this.DesignMode ? 10 : 1);
		}
		...
		internal int GetChangeSizeHeight(bool reduceSize)
		{
			...
			{
                ButtonItem button = (ButtonItem)referenceItem;
				Size size=button.ImageFixedSize;
				button.ImageFixedSize=Size.Empty;
                eButtonStyle buttonStyle = button.ButtonStyle;
                button.ButtonStyle = eButtonStyle.ImageAndText;
				referenceItem.RecalcSize();
                
                height = Math.Max(height, referenceItem.HeightInternal + ItemPaddingBottom + ItemPaddingTop);
				
				button.ImageFixedSize=size;
