...
namespace DevComponents.DotNetBar
{
    ...
    [Designer("DevComponents.DotNetBar.Design.SuperTabStripDesigner, DevComponents.DotNetBar.Design, Version=14.1.0.25, Culture=neutral,  PublicKeyToken=90f470f34c89ccaf")]
    public class SuperTabStrip : ItemControl, ISupportInitialize
    {
        ...
        /// </summary>
        [Editor("DevComponents.DotNetBar.Design.SuperTabStripTabsEditor, DevComponents.DotNetBar.Design, Version=14.1.0.25, Culture=neutral,  PublicKeyToken=90f470f34c89ccaf", typeof(System.Drawing.Design.UITypeEditor))]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (e.Handled == false)
            {
                switch (e.KeyData)
                {
                    ...
                    case Keys.Up:
                    case Keys.Down:
                        switch (GetKeyDir(e.KeyData))
                        {
                            case KeyDir.Previous:
                                if (SelectPreviousTab() == true)
                                    e.Handled = true;
                                break;

                            case KeyDir.Next:
                                if (SelectNextTab() == true)
                                    e.Handled = true;
                                break;
                        }
                        break;
                    ...
                    case Keys.Home:
                        SuperTabItem ftab = TabStripItem.FirstVisibleTab;

                        if (ftab != null)
                        {
                            TabStripItem.SelectTab(ftab, eEventSource.Keyboard);
                            e.Handled = true;
                        }
                        break;

                    case Keys.End:
                        SuperTabItem ltab = TabStripItem.LastVisibleTab;

                        if (ltab != null)
                        {
                            TabStripItem.SelectTab(ltab, eEventSource.Keyboard);
                            e.Handled = true;
                        }
                        break;
                }
        ...
        private KeyDir GetKeyDir(Keys keys)
        {
            switch (keys)
            {
                case Keys.Left:
                    ...
                    {
                        if ((TabStripItem.TabAlignment == eTabStripAlignment.Left && TabStripItem.RotateVerticalText == false) ||
                            (TabStripItem.TabAlignment == eTabStripAlignment.Right && TabStripItem.RotateVerticalText == true))
                        {
                            return (KeyDir.Next);
                        }
                    }

                    return (KeyDir.Previous);
