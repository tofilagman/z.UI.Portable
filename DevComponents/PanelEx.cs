...
namespace DevComponents.DotNetBar
{
	...
	/// </summary>
    [ToolboxItem(true), Designer("DevComponents.DotNetBar.Design.PanelExDesigner, DevComponents.DotNetBar.Design, Version=14.1.0.25, Culture=neutral,  PublicKeyToken=90f470f34c89ccaf"), DefaultEvent("Click")]
    public class PanelEx : System.Windows.Forms.Panel, IButtonControl, Controls.INonClientControl, IScrollBarOverrideSupport
	{
			...
			else
			{
                ...
                else
                {
                    ...
                    if (m_RenderText)
                    {
                        ...
                        if (this.AutoScroll)
                        {
                            m_TextMarkup.Bounds = new Rectangle(-_TextMarkupLayoutMargin.Width + this.AutoScrollPosition.X,
                                -_TextMarkupLayoutMargin.Height+this.AutoScrollPosition.Y,
                                m_TextMarkup.Bounds.Width, m_TextMarkup.Bounds.Height);
                        }
	    ...
	    private bool _ResizeMarkupDelayed = false;
	    protected override void OnHandleCreated(EventArgs e)
	    {
	        if (_ResizeMarkupDelayed)
	        {
	            ResizeMarkup();
	        }
	    ...
	    private Size _TextMarkupLayoutMargin = new Size(-2, -2);
        protected virtual void ResizeMarkup()
        {
            if (m_TextMarkup != null)
            {
                if (!BarFunctions.IsHandleValid(this))
                {
                    _ResizeMarkupDelayed = true;
                    return;
                }
                _ResizeMarkupDelayed = false;
                Rectangle r = this.ClientTextRectangle;
                r.Inflate(_TextMarkupLayoutMargin);
                r.Inflate(-(m_Style.MarginLeft + m_Style.MarginRight), -(m_Style.MarginTop + m_Style.MarginBottom));
                if (this.IsThemed)
                    r.Height -= 4;
                if (r.Width <= 0 || r.Height <= 0)
        ...
        public VScrollBarAdv VScrollBar
        {
            get
            {
                if (_ScrollSkinner!=null)
                    return _ScrollSkinner.VScrollBar;
                return null;
            }
        }
        ...
        public HScrollBarAdv HScrollBar
        {
            ...
            {
                if (_ScrollSkinner != null)
                    return _ScrollSkinner.HScrollBar;
                return null;
            }
        }

        private void DisposeScrollBarSkinner()
        {
            if (_ScrollSkinner != null)
            {
        ...
        private bool _UseThemedScrollbars=true;
        /// <summary>
        /// Indicates whether panel uses themed DotNetBar scrollbars. Default value is true.
        /// </summary>
        [DefaultValue(true), Category("Behavior"), Description("Indicates whether panel uses themed DotNetBar scrollbars.")]
        public bool UseThemedScrollbars
        {
            get
            {
                return _UseThemedScrollbars;
            }
            set
            {
                if (_UseThemedScrollbars != value)
                {
                    DisposeScrollBarSkinner();
                    _UseThemedScrollbars = value;
                    if (_UseThemedScrollbars)
                    {
                        _ScrollSkinner = new ScrollbarSkinner(this);
                        _ScrollSkinner.ScrollBarAppearance = _ScrollBarAppearance;
                    }
                }
            }
        }
