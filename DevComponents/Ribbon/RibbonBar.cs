...
namespace DevComponents.DotNetBar
{
	...
	/// </summary>
    [ToolboxBitmap(typeof(RibbonBar), "Ribbon.RibbonBar.ico"), ToolboxItem(true), Designer("DevComponents.DotNetBar.Design.RibbonBarDesigner, DevComponents.DotNetBar.Design, Version=14.1.0.25, Culture=neutral,  PublicKeyToken=90f470f34c89ccaf"), System.Runtime.InteropServices.ComVisible(false)]
	public class RibbonBar:ItemControl
	{
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                this.ItemAdded -= RibbonBar_ItemAdded;
                this.ItemRemoved -= RibbonBar_ItemRemoved;

                m_TitleStyle.StyleChanged -= ElementStyleChanged;
                m_TitleStyle.Dispose();
                m_TitleStyleMouseOver.StyleChanged -= ElementStyleChanged;
                m_TitleStyleMouseOver.Dispose();
                m_BackgroundMouseOverStyle.StyleChanged -= this.VisualPropertyChanged;
                m_BackgroundMouseOverStyle.Dispose();

                m_DefaultBackgroundStyle.Dispose();
                m_DefaultBackgroundMouseOverStyle.Dispose();
                m_DefaultTitleStyle.Dispose();
                m_DefaultTitleStyleMouseOver.Dispose();
        ...
        private bool ResizeGallery(int contentBasedWidth)
        {
            ...
            if (m_GalleryStretch.DisplayRectangle.Width + diff >= m_GalleryStretch.MinimumSize.Width)
            {
                ...
                m_GalleryStretch.RecommendedSize = new Size(m_GalleryStretch.DisplayRectangle.Width + diff, Dpi.Height(m_GalleryStretch.DefaultSize.Height));
        ...
        private void AutoReduceItemSize(int contentBasedWidth)
        {
            ...
            if (m_AutoSizeBagList.Count > 0) _AutoSizeIncludesTitleEnabled = false;
        }
        ...
        internal void RestoreAutoSizedItems()
        {
            ...
            if (m_AutoSizeBagList.Count == 0) return;
            _AutoSizeIncludesTitleEnabled = true;
        ...
        {
            //Console.WriteLine("Name: {0}  Size:{1}", this.Name, this.Size);
        ...
        private bool _AutoSizeIncludesTitleEnabled = true;
        private Size GetControlSize(Size innerContentSize)
        {
            ElementStyle style = GetBackgroundStyle();

            if (_AutoSizeIncludesTitle && _AutoSizeIncludesTitleEnabled)
            {
                innerContentSize.Width = Math.Max(innerContentSize.Width, m_TitleElement.TextSize.Width + (m_DialogLauncherVisible ? 16 : 1));
            }
        ...
        private bool _AutoSizeIncludesTitle = false;
        /// <summary>
        ...
        /// </summary>
        [DefaultValue(false), Category("Behavior"), Description("Indicates whether auto-sizing for RibbonBar includes ribbon bar title width so in case when ribbon bar title width is wider than ribbon bar width the width of the ribbon bar will always be at least the title bar width so complete title text can be displayed.")]
        public bool AutoSizeIncludesTitle
        {
            get { return _AutoSizeIncludesTitle; }
            set
            {
                _AutoSizeIncludesTitle = value;
            }
        }
		...
		private void PaintDialogLauncher(ItemPaintArgs pa)
		{
            ...
            if (BarFunctions.IsOffice2007Style(this.EffectiveStyle))
            {
                int ds = Dpi.Height((int)Math.Min(15, m_TitleRectangle.Height - 1));
