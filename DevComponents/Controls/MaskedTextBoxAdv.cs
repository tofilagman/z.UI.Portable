...
namespace DevComponents.DotNetBar.Controls
{
    ...
    public class MaskedTextBoxAdv : PopupItemControl, IInputButtonControl, ICommandSource
    {
        ...
        private void UpdateEnabledBackColor()
        {
            if (!_DisabledBackColor.IsEmpty)
            {
                if (!this.Enabled)
                {
                    ...
                    _MaskedTextBox.BackColor = _DisabledBackColor;
                }
                else if (!_EnabledBackColor.IsEmpty)
                {
                    this.BackColor = _EnabledBackColor;
                    _MaskedTextBox.BackColor = _EnabledBackColor;
        ...
        /// </summary>
        [RefreshProperties(RefreshProperties.Repaint), Editor("DevComponents.DotNetBar.Design.MaskAdvPropertyEditor, DevComponents.DotNetBar.Design, Version=14.1.0.25, Culture=neutral,  PublicKeyToken=90f470f34c89ccaf", typeof(UITypeEditor)), Description("Indicates input mask to use at run time."), MergableProperty(false), Category("Behavior"), DefaultValue(""), Localizable(true)]
        public string Mask
        {
            get
            {
        ...
        private class MaskedTextBoxInternal : MaskedTextBox
        {
            ...
            protected override void WndProc(ref Message m)
            {
                ...
                switch (m.Msg)
                {
                    ...
                    //    {
                    //        if (!this.Enabled && this.Parent is MaskedTextBoxAdv && !((MaskedTextBoxAdv)this.Parent).DisabledBackColor.IsEmpty)
                    //        {
                    //            Rectangle r = this.ClientRectangle;
