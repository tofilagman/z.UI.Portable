...
{
    public class TabParentForm : Form
    {
        ...
        #region Constructor
        private System.Windows.Forms.Padding GlassDefaultPadding = new System.Windows.Forms.Padding(1, 1, 1, 1); //new System.Windows.Forms.Padding(0, 1, 1, 1);
        ...
        protected override void OnShown(EventArgs e)
        {
            if (this.DesignMode)
                UpdateColorScheme();
            else if (_PreRenderShell && _FormTabs != null)
            {
                using (Bitmap bmp = new Bitmap(16, 16))
                    _FormTabs.DrawToBitmap(bmp, new Rectangle(0, 0, this.Width, this.Height));
            }
            base.OnShown(e);
        }
        ...
        private void ShowModalPanel(Control panel, bool slideIn, eSlideSide slideFromSide, int animationTimeMilliseconds)
        {
            ...
            if (_ModalPanels.Count == 1 && _FormTabs != null)
            {
                //_FormTabs.MetroTabStrip.Enabled = false;
                _FormTabs.TabStrip.StripContainerItem.Enabled = false;
                for (int i = 0; i < _FormTabs.TabStrip.CaptionContainerItem.SubItems.Count; i++)
                {
                    BaseItem item = _FormTabs.TabStrip.CaptionContainerItem.SubItems[i];
                    if (item.SystemItem && !(item is QatCustomizeItem)) continue;
                    item.Enabled = false;
                    _DisabledItemIds.Add(item.Id);
                }
                _FormTabs.TabStrip.Invalidate();
            }
        ...
        private void CloseModalPanel(Control panel, bool slideOut, eSlideSide slideOutToSide)
        {
            ...
            if (_ModalPanels.Count == 0 && _FormTabs != null)
            {
                //_FormTabs.MetroTabStrip.Enabled = true;
                _FormTabs.TabStrip.StripContainerItem.Enabled = true;
                for (int i = 0; i < _FormTabs.TabStrip.CaptionContainerItem.SubItems.Count; i++)
                {
                    BaseItem item = _FormTabs.TabStrip.CaptionContainerItem.SubItems[i];
        ...
        private Rectangle GetModalPanelBounds()
        {
            ...
            if (_FormTabs != null && _FormTabs.CaptionVisible)
            {
                int captionHeight = _FormTabs.TabStrip.GetCaptionHeight() + 2;
        ...
        {
            UpdateSystemCaptionItem();
            base.OnStyleChanged(e);
        }

        protected override void OnControlAdded(ControlEventArgs e)
        {
            if (e.Control is TabFormControl)
            {
                _FormTabs = e.Control as TabFormControl;
                UpdateSystemCaptionItem();
            }
        ...
        public Thickness GetBorderThickness()
        {
            ...
            TabFormColorTable ct = GetFormColorTable();

            return ct.BorderThickness;
        }

        private TabFormColorTable GetFormColorTable()
        {
            if (_FormTabs != null && _FormTabs.ColorTable != null)
                return _FormTabs.ColorTable;
            Office2007ColorTable table = ((Office2007Renderer) GlobalManager.Renderer).ColorTable;
            return table.TabForm;
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            if (!_IsGlassEnabled && this.Region == null)
                this.Region = GetPlainFormRegion();
            Office2007Renderer renderer = GetRenderer();
            Graphics g = e.Graphics;
            renderer.DrawTabParentForm(new TabFormPainterArgs(this, g));
            base.OnPaint(e);
        }

        private Office2007Renderer GetRenderer()
        {
            return ((Office2007Renderer) GlobalManager.Renderer);
        }
        ...
        private void ExtendGlass()
        {
            ...
            m.cxLeftWidth = 0;
            m.cxRightWidth = 0;
            m.cyBottomHeight = NativeFunctions.IsTerminalSession() ? 0 : 1;
        ...
        protected virtual bool WindowsMessageSetText(ref Message m)
        {
            ...
            if (_FormTabs != null) _FormTabs.Refresh();
            if (modified)
            {
        ...
        protected virtual bool WindowsMessageSetCursor(ref Message m)
        {
            ...
            if (_FormTabs != null && this.DesignMode) _FormTabs.Refresh();
            if (modified)
            {
                WinApi.ModifyHwndStyle(m.HWnd, 0, (int)WinApi.WindowStyles.WS_VISIBLE);
            }
        ...
        protected virtual bool WindowsMessageNCHitTest(ref Message m)
        {
            ...
            if (/*BarFunctions.IsWindows7 && this.WindowState == FormWindowState.Maximized &&*/ tab != null)
            {
                p = tab.TabStrip.PointToClient(new Point(x, y));
                if (tab.TabStrip.CaptionBounds.Contains(p) || tab.TabStrip.HitTest(p.X, p.Y) == null)
                {
        ...
        {
            if (_FormTabs != null)
                _FormTabs.TabStrip.Invalidate(_FormTabs.TabStrip.CaptionBounds);
        }
        ...
        protected virtual bool WindowsMessageNCCalcSize(ref Message m)
        {
            ...
            if (m.WParam != IntPtr.Zero && GetWindowState() == FormWindowState.Maximized)
            {
                ...
                if (_AutoHideTaskBarReduction > 0)
                {
                    Taskbar taskBar = new Taskbar();
                    if (taskBar.AutoHide)
                    {
                        if (taskBar.Position == TaskbarPosition.Bottom)
                            newClientRect.Bottom -= _AutoHideTaskBarReduction;
                        else if (taskBar.Position == TaskbarPosition.Top)
                        {
                            newClientRect.Bottom -= _AutoHideTaskBarReduction;
                            newClientRect.Top += _AutoHideTaskBarReduction;
                        }
                        else if (taskBar.Position == TaskbarPosition.Left)
                        {
                            newClientRect.Right -= _AutoHideTaskBarReduction;
                            newClientRect.Left += _AutoHideTaskBarReduction;
                        }
        ...
        private int _AutoHideTaskBarReduction = 4;
        /// <summary>
        ...
        public int AutoHideTaskBarReduction
        {
            get { return _AutoHideTaskBarReduction; }
            set { _AutoHideTaskBarReduction = value; }
        }
        ...
        private void UpdateSystemCaptionItem()
        {
        ...
        private SystemCaptionItem GetSystemCaptionItem()
        {
            if (_FormTabs != null)
            {
                return _FormTabs.TabStrip.SystemCaptionItem;
            }
            return null;
        }
        ...
        public bool CloseBoxVisible
        {
            ...
            set { _CloseBoxVisible = value; UpdateSystemCaptionItem(); }
        }
