...
namespace DevComponents.DotNetBar
{
    ...
    public class ElementStyleDisplay
    {
        ...
        public static void PaintText(ElementStyleDisplayInfo e, string text, Font defaultFont, bool useDefaultFont, eTextFormat textFormat)
        {
            ...
            textBounds.X += style.MarginLeft + style.PaddingLeft;
            textBounds.Width -= (style.MarginLeft + style.MarginRight + style.PaddingLeft + style.PaddingRight);
            textBounds.Y += style.MarginTop + style.PaddingTop;
            textBounds.Height -= (style.MarginTop + style.MarginBottom + style.PaddingTop + style.PaddingBottom);
        ...
        public static void PaintBackground(ElementStyleDisplayInfo e, bool shapeBackground)
        {
            ...
            if (shapeBackground && !IsSquareCornerType(style))
            {
        ...
        private static bool IsSquareCornerType(ElementStyle style)
        {
            return (style.CornerType == eCornerType.Square || style.CornerType == eCornerType.Inherit) &&
                   (style.CornerTypeTopLeft == eCornerType.Square || style.CornerTypeTopLeft == eCornerType.Inherit) &&
                   (style.CornerTypeTopRight == eCornerType.Square || style.CornerTypeTopRight == eCornerType.Inherit);
        }
            ...
            try
            {
                ...
                switch (imagePosition)
                {
                    ...
                    case eStyleBackgroundImage.Center:
                        {
                            Rectangle destRect = new Rectangle(bounds.X, bounds.Y, Dpi.Width(backgroundImage.Width),
                                                               Dpi.Height(backgroundImage.Height));
                            if (bounds.Width > destRect.Width)
                                destRect.X += (bounds.Width - destRect.Width) / 2;
                            if (bounds.Height > destRect.Height)
                                destRect.Y += (bounds.Height - destRect.Height) / 2;
                            if (imageAtt != null)
                    ...
                    case eStyleBackgroundImage.BottomRight:
                        {
                            ...
                            if (imagePosition == eStyleBackgroundImage.TopRight)
                                destRect.X = bounds.Right - destRect.Width;
                            else if (imagePosition == eStyleBackgroundImage.BottomLeft)
                                destRect.Y = bounds.Bottom - destRect.Height;
                            else if (imagePosition == eStyleBackgroundImage.BottomRight)
                            {
                                destRect.Y = bounds.Bottom - destRect.Height;
                                destRect.X = bounds.Right - destRect.Width;
                            ...
                            if (imageAtt != null)
                            {
                                if (bounds.Width > backgroundImage.Width || bounds.Height > backgroundImage.Height)
                                {
                                    ...
                                    while (y < bounds.Bottom)
                                    {
                                        ...
                                        {
                                            Rectangle destRect = new Rectangle(x, y, Dpi.Width(backgroundImage.Width),
                                                                               Dpi.Height(backgroundImage.Height));
                                            ...
                                            x += Dpi.Width(backgroundImage.Width);
                                        }
                                        x = bounds.X;
                                        y += Dpi.Height(backgroundImage.Height);
                                    }
                                }
                                else
                                {
                                    e.Graphics.DrawImage(backgroundImage,
                                                         new Rectangle(0, 0, Dpi.Width(backgroundImage.Width),
                                                                       Dpi.Height(backgroundImage.Height)), 0, 0,
