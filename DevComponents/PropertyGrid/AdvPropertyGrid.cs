    ...
    public class AdvPropertyGrid : ContainerControl, INotifyPropertyChanged, IPropertyGridLocalizer, IPropertyElementStyleProvider, ISupportInitialize
    {
        ...
        private void UpdatePropertyValue(Node item, string propertyName)
        {
            if (this.InvokeRequired)
                this.BeginInvoke(new MethodInvoker(delegate { UpdatePropertyValueInternal(item, propertyName); }));
            else
                UpdatePropertyValueInternal(item, propertyName);
        }

        private void UpdatePropertyValueInternal(Node item, string propertyName)
        {
